...
class LLaDABlock(nn.Module):
    ...
    def _scaled_dot_product_attention(
        ...
        def chebyshev_fit_exp(a, b, degree=10, n_samples=4096):
            """
            Fit exp(x) on [a,b] using Chebyshev polynomial basis.

            Returns:
                coeffs: numpy array shape [degree+1]
            """
            # Chebyshev nodes in [-1,1]
            t = np.cos(np.pi * (np.arange(n_samples) + 0.5) / n_samples)
            ...
            # fit in Chebyshev basis on [-1,1]
            coeffs = chebfit(t, y, deg=degree)

            return coeffs

        def chebyshev_eval_exp(X, coeffs, a, b):
            """
            Evaluate Chebyshev approximation of exp(X) on [a,b].

            X: numpy array or torch tensor (will convert to numpy)
            coeffs: chebyshev coeffs from chebyshev_fit_exp
            """
            import torch
            ...
            # map X from [a,b] -> [-1,1]
            t_np = (2 * X_np - (b + a)) / (b - a)
            t_np = np.clip(t_np, -1.0, 1.0)

            # evaluate polynomial
            out_np = chebval(t_np, coeffs)

            if is_torch:
                return torch.tensor(out_np, device=device, dtype=dtype)
            else:
                return out_np

        def approximate(X, a, b, degree):
            coeffs = chebyshev_fit_exp(a, b, degree=degree)
            approx = chebyshev_eval_exp(X, coeffs, a, b)
            return approx


        def top_bottom_k_percent_mask_and_clamp(A, k=0.1):
            """
            A: Tensor of shape [B, C, H, W]
            k: fraction (e.g. 0.1 means top 10% and bottom 10%)

            Returns:
                mask: same shape as A, dtype=torch.bool
                A_clamped: same shape as A, same dtype
            """
            B, C, H, W = A.shape
            N = H * W
            flat = A.reshape(B, C, N)

            m = int(k * N)
            if m < 1:
                raise ValueError("k is too small, results in zero selected elements")

            bottom_vals, _ = torch.kthvalue(flat, k=m, dim=-1)  # [B,C]
            top_vals, _ = torch.kthvalue(flat, k=N - m + 1, dim=-1)  # [B,C]

            bottom_vals = bottom_vals.unsqueeze(-1)  # [B,C,1]
            top_vals = top_vals.unsqueeze(-1)  # [B,C,1]

            bottom_mask = flat <= bottom_vals
            top_mask = flat >= top_vals
            mask = bottom_mask | top_mask

            flat_clamped = flat.clone()
            flat_clamped[bottom_mask] = bottom_vals.expand_as(flat)[bottom_mask]
            flat_clamped[top_mask] = top_vals.expand_as(flat)[top_mask]

            return mask.reshape(B, C, H, W), flat_clamped.reshape(B, C, H, W)
        ...
        if self.flash_attn_func is not None and attn_mask is None:
            ...
            # code starts
            exact_scores = torch.matmul(q, k.transpose(-2, -1))  # (B,H,L,L)
            D = q.shape[-1]
            exact_scores = exact_scores / math.sqrt(D)

            # max_scores = exact_scores.max(dim=-1, keepdim=True).values
            # exact_scores = exact_scores - max_scores

            exact_proportion = 0.2
            chebyshev_degree = 6
            exact_mask, clipped_exact_scores = top_bottom_k_percent_mask_and_clamp(exact_scores, k=exact_proportion/2)
            exact_mask = exact_mask.to(dtype=clipped_exact_scores.dtype, device=clipped_exact_scores.device)

            #####
            a = float(clipped_exact_scores.min().item())
            b = float(clipped_exact_scores.max().item())
            approx = approximate(clipped_exact_scores, a, b, degree=chebyshev_degree)
            # exact = torch.exp(exact_scores)

            # approx_mask = (exact_mask == 0)
            # approx = approx[approx_mask]
            # exact = exact[approx_mask]
            # rel_error = ((approx - exact).abs() / (exact.abs() + 1e-8)).mean()
            ...
            # approximated_scores = chebyshev_approximate_per_head(clipped_exact_scores, degree=chebyshev_degree)
            #
            merged_score = torch.exp(exact_scores) * exact_mask + approx * (1 - exact_mask)

            # approx_mask = (exact_mask == 0)
            # ref = torch.exp(clipped_exact_scores)  # ✅ 近似目标
            # rel_err = ((approximated_scores - ref).abs() / (ref.abs() + 1e-6))
            # error = rel_err[approx_mask].mean()
            # print("rel_err (approx region):", error.item())
            # breakpoint()
            attn = merged_score / merged_score.sum(dim=-1, keepdim=True)
            # exact_scores = torch.exp(exact_scores)
            # exact_attn = exact_scores / exact_scores.sum(dim=-1, keepdim=True)
            # rel_error = (attn - exact_attn).abs().mean()
            # print("rel_err (approx region):", rel_error.item())
            # breakpoint()

            if dropout_p > 0 and model.training:
    ...
    def scaled_dot_product_attention_hybrid(
            q: torch.Tensor,
            k: torch.Tensor,
            v: torch.Tensor,
            attn_mask: Optional[torch.Tensor] = None,
