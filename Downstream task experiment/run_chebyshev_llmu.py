...
Answer with the letter (A, B, C, or D) that best answers the question.
Answer:"""
    return prompt
...
def chat_hellaswag(args, steps, block_length):
    ...
    for idx, datum in enumerate(tqdm.tqdm(dataset)):
        context = datum["question"]
        endings = datum["choices"]
        correct_label = int(datum["answer"])
    ...
    out_dir = Path(f"llmu/steps{steps}_block{block_length}_exact{exact_proportion}_degree{chebyshev_degree}")
    out_dir.mkdir(parents=True, exist_ok=True)
    with open(out_dir / "results.json", "w") as f:
        json.dump(results, f, indent=2)

    print(f"MMLU Accuracy: {100 * correct / total:.2f}%")
